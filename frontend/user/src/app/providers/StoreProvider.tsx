...
export const StoreProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [addresses, setAddresses] = useState<Address[]>(() => {
    const stored = localStorage.getItem('kutuku_addresses');
    if (stored) return JSON.parse(stored);
    return [
      {
        id: 'addr-1',
        title: 'Main',
        street: 'الحميد, RPFM+G2 - بلجرشي',
        city: 'بلجرشي',
        country: 'Saudi Arabia',
        zipCode: '65611',
        recipientName: 'Mohammed Alghamdi',
        recipientPhone: '+966-53-1061101'
      },
      {
        id: 'addr-2',
        title: 'Main',
        street: 'قريه الحميد, RPFM+H4 - بلجرشي',
        city: 'بلجرشي',
        country: 'Saudi Arabia',
        zipCode: '65611',
        recipientName: 'ahmed alghamdi',
        recipientPhone: '+966-53-5887767'
      }
    ];
  });
  ...
  useEffect(() => {
    localStorage.setItem('kutuku_addresses', JSON.stringify(addresses));
  }, [addresses]);
  ...
  const updateAddress = (id: string, updatedFields: Address) => {
    setAddresses(prev => prev.map(addr => addr.id === id ? { ...addr, ...updatedFields } : addr));
    if (selectedAddress?.id === id) {
      setSelectedAddressState(prev => prev ? { ...prev, ...updatedFields } : null);
    }
  };
  ...
  const deleteAddress = (id: string) => {
    ...
    if (selectedAddress?.id === id) {
      ...
      setSelectedAddressState(remaining.length > 0 ? remaining[0] : null);
    }
  };
